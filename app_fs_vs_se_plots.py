...
se = [float(line.rstrip('\n')) * 1000000 for line in open('se_app.txt')]
fs = [float(line.rstrip('\n')) * 1000000 for line in open('fs_app.txt')]
names = [line for line in open('app_names.txt')]
...
ax.set_title('Relative simulated time by emulation mode - app benchmarks')
ax.set_xticks(ind)
...
#plt.show()
plt.savefig('app_fs_vs_se_plots.png')
