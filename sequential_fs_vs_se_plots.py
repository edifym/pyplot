    ...
    *xpos* indicates which side to place the text w.r.t. the center of
    ...
    for rect in rects:
        ...
        ax.annotate('{}%'.format(height),
                    ...
                    xytext=(offset[xpos]*-10, 3),  # use 3 points offset
                    textcoords="offset points",  # in both directions
                    ha=ha[xpos], va='bottom')


se = [float(line.rstrip('\n')) * 1000000 for line in open('se_sequential.txt')]
fs = [float(line.rstrip('\n')) * 1000000 for line in open('fs_sequential.txt')]
names = [line for line in open('sequential_names.txt')]
...
ax.set_title('Relative simulated time by emulation mode - sequential benchmarks')
ax.set_xticks(ind)
...
#plt.show()
plt.savefig('sequential_fs_vs_se_plots.png')
